...
3. `status: "dummy"` 사례를 실제 사례로 교체·확충 — `case-001`(성공)·`case-002`(성공)·`case-003`
   (지연)·`case-004`(부도)·`case-006`(부도)·`case-007`(부도) 완료(공공기관 공시자료·언론보도·
   HUG 공매정보 CSV 근거, 아래 "주의" 참고). `case-005`("정보 미확정 초기 검토" 시나리오 — 애초에
   실제 사례보다 더미로 남기는 게 더 적합한 유형)만 더미로 유지. 총 7건(더미 1 + 실제 6)

## 주의

- `case-005`를 제외한 나머지 6개 사례는 **실제 사례**입니다(`status: "verified"`). `case-005`만
...
- `case-006`·`case-007` 모두 AI등급(`BBB`대)이 실제 결과(`default`)와 **불일치**로 나옵니다 —
  두 사례 다 재무조건이 비공개라 앱 기본 미정값(자기자본 20%·금리 9%·27개월 등, "안전해 보이는"
  값)을 썼기 때문에 하드 게이트가 걸리지 않은 것으로 보이며, 이는 채점모델의 한계를 보여주는
  의미 있는 검증 결과입니다(버그 아님 — 재무조건 미공개 상황에서의 판정 한계).
