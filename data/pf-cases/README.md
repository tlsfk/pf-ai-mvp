...
3. `status: "dummy"` 사례를 실제 사례로 교체 — `case-001`(성공)·`case-004`(부도) 완료(공공기관
   공시자료·언론보도 근거, 아래 "주의" 참고). 나머지 3건(`case-002`·`case-003`·`case-005`)은 아직 더미

## 주의

- `case-001`·`case-004`를 제외한 나머지 3개 사례는 **더미 데이터**입니다(`status: "dummy"`,
  `source`에 명시). 실제 사업지·시행사 정보가 아니므로 그대로 실서비스 근거로 쓰면 안 됩니다.
