...
3. `status: "dummy"` 사례를 실제 사례로 교체 — `case-001`(성공)·`case-002`(성공)·`case-003`(지연)·
   `case-004`(부도) 완료(공공기관 공시자료·언론보도 근거, 아래 "주의" 참고). 나머지 1건(`case-005`,
   "정보 미확정 초기 검토" 시나리오 — 애초에 실제 사례보다 더미로 남기는 게 더 적합한 유형)만 더미로 유지

## 주의

- `case-005`를 제외한 나머지 4개 사례는 **실제 사례**입니다(`status: "verified"`). `case-005`만
